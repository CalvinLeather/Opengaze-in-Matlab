function [secs, keyCode, deltaSecs, gp] = KbWaitEyeTrack(deviceNumber, forWhat, untilTime, obj)
% [secs, keyCode, deltaSecs, gp] = KbWaitEyetrack([deviceNumber][, forWhat=0][, untilTime=inf])
%This is adapted from psychtoolbox code, you need psychtoolbox to run it.
%Notes for custom- so far, rt and length(gp) doesn't line up. Enabling the
%counter suggests that packets at the beginning or end are being missed,
%rather than packets being missed occasionally in the middle. Either its
%not collecting them quickly enough, or its not waiting long enough at the
%end. Turn on counter to test.
%
...
while secs < untilTime
    ...
    if (get(obj.client_socket, 'BytesAvailable') > 0)                               
